...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size, positional, dropout_p=0.1):
        ...
        self.word_embedding = nn.Embedding(input_size, hidden_size).cuda()
        self.pos_embedding = nn.Embedding(input_size, hidden_size).cuda()
...
def sentence_to_tensor(w2i, sentence):
    ...
    return torch.cuda.tensor(indices, dtype=torch.long).view(-1, 1)
...
def train(input_sentence, target_sentence, w2i_english,
    ...
    decoder_input = torch.cuda.tensor([[SOS_token]])
...
encoder = EncoderRNN(len(i2w_english), 256, positional).cuda()
decoder = AttnDecoderRNN(256, len(i2w_french)).cuda()
...
def evaluate(encoder, decoder, w2i_english, i2w_french,
    ...
    with torch.no_grad():
        ...
        decoder_input = torch.cuda.tensor([[SOS_token]])  # SOS
